...
#ifdef LEFT_MOTOR
#define VMAX 79.55   //   0.22m/s=((2*pi*VMAX)/60)*32.5*10^-3 (r)
#endif

#ifdef RIGHT_MOTOR
#define VMAX 80.1  //   0.22m/s=((2*pi*VMAX)/60)*32.5*10^-3 (r)
#endif
...
#ifdef RIGHT_MOTOR
//if conditions were determined using k-map
...
void loop() {
  ...
  while (start_stop == 7) {
#ifdef RIGHT_MOTOR
    analogWrite(PWM, 150); // Motor speed
#else
    analogWrite(PWM, 148); // Motor speed
#endif
    digitalWrite(IN1, HIGH);
    digitalWrite(IN2, LOW);
    nh.spinOnce();
  ...
  if (start_stop == 4 || start_stop == 6) {
    pwr = (int) fabs(u);
  }
  if (start_stop == 8 || (camera == 10 && start_stop != 6)) {
    pwr = 0;
    eintegral = 0;
    if (camera == 10 && start_stop != 6)
    {
  ...
  eprev = e;
  if (start_stop == 6 && ultrasonic2.read() < 40) {
    pwr = 0;
    eintegral = 0;
  }
